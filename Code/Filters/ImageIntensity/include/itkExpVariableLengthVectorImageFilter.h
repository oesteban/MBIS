...
public:
  ...
  {
    size_t vLength = itk::NumericTraits< TInput >::GetLength( A );
    TOutput out;
    itk::NumericTraits< TOutput >::SetLength( out, vLength );

    for( size_t i = 0; i < vLength; i++ ) {
        out[i] = static_cast< typename TOutput::ValueType >( vcl_exp( static_cast< double >( A[i] ) ) );
    }
...
public:
  ...
  itkConceptMacro( InputConvertibleToDoubleCheck,
                   ( Concept::Convertible< typename TInputImage::PixelType::ValueType, double > ) );
  itkConceptMacro( DoubleConvertibleToOutputCheck,
                   ( Concept::Convertible< double, typename TOutputImage::PixelType::ValueType > ) );
